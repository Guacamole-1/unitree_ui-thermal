...
import { ThermalWebRTCConnection } from '../connection/thermal_webrtc';
import { PipThermal } from './components/pip-thermal';
import {GimbalClient} from '../connection/gimbal'
...
export class App {
	...
	private thermalwebrtc : ThermalWebRTCConnection | null = null;
	private dataHandler: DataChannelHandler | null = null;
	private videoStream: MediaStream | null = null;
	private thermalStream: MediaStream | null = null;
	private connectionConfig: ConnectionConfig | null = null;
	private gimbalClient: GimbalClient | null = null;

	// Control UI components
	private navBar: NavBar | null = null;
	private pipCamera: PipCamera | null = null;
	private pipThermal: PipThermal | null = null;
	private useThermalCam: boolean = false;
	...
	// robot speed for settings bar
	private movementSpeed = 1.0; // 0.0 to 1.0
	private turnSpeed = 0.5;
	...
	private showConnectionScreen(): void {
		...
		(window as any).connectionPanel = this.connectionPanel;
	}
	...
	private showControlUi(): void {
		...
		// thermal camera
		// todo: toggleviewmode
		if(this.useThermalCam){
		  this.pipThermal = new PipThermal(this.controlUi);
		  if (this.thermalStream) {
			  this.pipThermal.setStream(this.thermalStream);
		  }
		  this.pipThermal.setOnTap(() => this.toggleViewMode());
		  this.gimbalClient = new GimbalClient()
		}
		...
		this.settingBar = new SettingBar(this.controlUi, {
			...
			onMovementSpeedSet: (level) => {if (level >= 0 && level <= 1.0) this.movementSpeed = level},
			onTurnSpeedSet: (level) => {if (level >= 0 && level <= 1.0) this.turnSpeed = level},
			onGimbalMove: (pan, tilt) => {this.gimbalClient?.move({ pan:pan, tilt:tilt });},
	...
	private goToHub(): void {
		...
		this.heldKeys.clear(); // avoid stuck keys if cleanup happens mid-press
		this.settingBar?.destroy(); // destroy gimbal listeners
		this.setRelay(false);
	...
	private startJoystickLoop(): void {
		this.joystickTimer = setInterval(() => {
			let { lx, ly, rx, ry } = this.joystickState;
			// keyboard movement
			if (this.heldKeys.has('w')) ly = this.movementSpeed;
			if (this.heldKeys.has('s')) ly = -this.movementSpeed;
			if (this.heldKeys.has('a')) lx = -this.movementSpeed;
			if (this.heldKeys.has('d')) lx = this.movementSpeed;
			if (this.heldKeys.has('ArrowUp'))    ry = this.turnSpeed;
			if (this.heldKeys.has('ArrowDown'))  ry = -this.turnSpeed;
			if (this.heldKeys.has('ArrowLeft'))  rx = -this.turnSpeed;
			if (this.heldKeys.has('ArrowRight')) rx = this.turnSpeed;
	...
	//  ── Keyboard Event Listener ──
	// class property — adjustable movement speed
	...
	private KeyboardMovementListener = (event: KeyboardEvent): void => {
		console.log("key pressed:", event.key)
		const movementSpeed = 0.5;
		let lx = 0, ly = 0, rx = 0, ry = 0;

		if (event.key === 'w') ly = movementSpeed;
		if (event.key === 's') ly = -movementSpeed;
		if (event.key === 'a') lx = -movementSpeed;
		if (event.key === 'd') lx = movementSpeed;

		if (event.key === 'ArrowUp')    ry = movementSpeed;
		if (event.key === 'ArrowDown')  ry = -movementSpeed;
		if (event.key === 'ArrowLeft')  rx = -movementSpeed;
		if (event.key === 'ArrowRight') rx = movementSpeed;
	...
	private async connect(config: ConnectionConfig): Promise<void> {
		...
		const thermalcallbacks: ConnectionCallbacks = {
			onStateChange: () => {},
			onValidated:  () => {
				this.connectionPanel?.setStatus('Thermal Camera Connected!','success');
				this.showHubScreen();
			},
			// display it on screen maybe?
			onMessage: () => {},
			onVideoTrack: (stream: MediaStream) => {
				this.thermalStream = stream;
				this.pipThermal?.setStream(stream);
			},
			onAudioTrack: () => {},
		...
		try {
			if (config.mode === 'STA-T') {
				...
				//turn this off only for debug!
				this.webrtc = await connectLocal(config.ip, config.mode, callbacks, onStep);
				// thermal camera only connects on local network for now
				console.log("Thermal camera connecting..");
				if(config.mode === 'STA-L' && config.thermal ) {
					this.useThermalCam = true;
					this.thermalwebrtc = new ThermalWebRTCConnection(thermalcallbacks);
					this.thermalwebrtc.onThermalData = (d) => {
						this.pipThermal?.updateStats(d);
					};
